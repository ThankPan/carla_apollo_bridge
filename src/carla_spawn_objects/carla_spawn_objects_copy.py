...
class CarlaSpawnObjects(CompatibleNode):
    ...
    def __init__(self):
        ...
        self.objects_definition_file = os.path.dirname(__file__) + '/config/objects.json'
        self.spawn_sensors_only = False # self.get_param('spawn_sensors_only', False)
    ...
    def setup_sensors(self, sensors, attached_vehicle_id=None):
        ...
        for sensor_spec in sensors:
            ...
            try:
                ...
                spawn_object_request.transform.CopyFrom(sensor_transform)
    ...
    def spawn_actors(self, destroy_command):
            ...
            if index < 20:
                try:
                    ...
                    for v in actor_vehicles:
                        destroy_command.append(carla.command.DestroyActor(v))
                except RuntimeError as e:
...
def main(args=None):
    ...
    if spawn_objects_node:
        try:
            spawn_objects_node.spawn_actors(destroy_commands)
            try:
                spawn_objects_node.spin()
            except (CyberInterruptException, ServiceException, KeyboardInterrupt):
                pass
        except (CyberInterruptException, ServiceException):
